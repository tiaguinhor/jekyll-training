...
# Modified by Tiago Calogero 2017
...
module Jekyll
  ...
  class DataPage < Page
    ...
    def initialize(site, base, index_files, dir, dir_name, data, name, template, extension)
      @site = site
      @base = base
      ...
      if index_files
        ...
        @name =  "index" + "." + extension.to_s
      	...
      	# create custom folder with dir_name field in posts.json
      	if dir_name
			new_dir = data[dir_name].split(' ')[0].split('-')
			# puts new_dir
			@dir = dir + "/" + new_dir[0] + "/" + new_dir[1] + "/" + new_dir[2]
		else
			@dir = dir
		end
        ...
        data.each do |data_spec|
          ...
          if site.layouts.key? template
            ...
            records.each do |record|
              site.pages << DataPage.new(site, site.source, index_files, dir, dir_name, record, name, template, extension)
            end
          else
